...
public sealed class BridgeService
{
    ...
    private IDgLabClient? _frontend;
    ...
    public void Start()
    {
        lock (_gate)
        {
            ...
            _server.Start();
            var protocol = (_config.Server.Protocol ?? "v3").ToLowerInvariant();
            if (protocol == "v4")
            {
                var v4 = new DgLabV4Client(this);
                _frontend = v4;
                v4.Start(_config.Server.V4Port);
            }
            else
            {
                var v3 = new DgLabFrontendClient(this);
                _frontend = v3;
                v3.Start(_config.Server.Port);
            }
