...
internal sealed class DgLabFrontendClient : IDgLabClient
{
    ...
    public async Task SendStrengthAsync(ChannelRef channel, StrengthOperation operation, int value)
    {
        ...
        int type = operation switch
        {
            StrengthOperation.Set => 3,
            StrengthOperation.DeltaUp => 2,
            StrengthOperation.DeltaDown => 1,
            StrengthOperation.Clear => 4,
            _ => 3
        };

        string message = operation == StrengthOperation.Clear
            ? $"clear-{(int)channel}"
            : "strength";

        int strengthValue = operation == StrengthOperation.Clear ? 0 : Math.Clamp(value, 0, 200);
        ...
        await SendEnvelopeAsync(new
        {
            ...
            clientId,
            targetId,
            channel = ((int)channel).ToString(),
            strength = strengthValue,
            message
        });
        ModLog.Info($"Sent strength message: channel={(int)channel}, type={type}, strength={strengthValue}");
    }
